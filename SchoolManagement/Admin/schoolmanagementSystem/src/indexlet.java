...
public class indexlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(reqid.equals("school"))
		{
			PreparedStatement st = con.prepareStatement("select * from schooladmin;");
			ResultSet rs = st.executeQuery();
			
		    out.print("<table align='center'>");
			out.print("<tr><td>Faculty ID	</td><td>Co-Host	</td><td>School</td></tr>");
		...
		else if(reqid.equals("stop"))
		{
		...
		else if(reqid.equals("studentSearch")) {
			String busNo= request.getParameter("studentNo");
			PreparedStatement st = con.prepareStatement("select * from z"
					+busNo+" order by class_time;");
			ResultSet rs = st.executeQuery();
			
		    out.print("<table align='center'>");
			out.print("<tr><td>STOP:</td><td>Fixed class Time</td>"
			...
			out.print("<hr size='2px'><div id='showbus' class = 'section'> <p class='heading'></p> 	 <br/> 	<div id='bustab'></div> </div> <hr size='2px'> <div id='showstops' class = 'section'> <p class='heading'>CLASS</p> 	 <br/> 	<div id='stoptab'></div> </div> <hr size='2px'> <div id='showz' class = 'section'> <p class='heading'>CLASS</p> <input id='routeSearch' type='text' placeholder='Enter students number'/> <button type='button' id='busSearch'>Search Student</button> <br/> 	<div id='searchtab'></div> </div><script> $(document).ready( function() { $.get('indexlet',{ 	'id': 'bus' }).done(function(response) {  $('#bustab').html(response); }); $.get('indexlet',{ 	'id': 'stop' }).done(function(response) { $('#stoptab').html(response); }); }); $('#busSearch').click( function() {  $.get('indexlet',{ 	'id': 'busSearch', 	'busNo': document.getElementById('routeSearch').value} ).done(function(response) {  	$('#searchtab').html(response);  }); }); </script>");
		}
