...
public class Sass extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try { 
            ...
            st.executeUpdate();
            st.close();
            response.getWriter().write("co-host changed");
             }
             else if(id.equals("print")){
            	 response.getWriter().write("<hr size='2px'> <div id='ass' class='section'> <p class='heading'>ASSOCIATE CO_HOST</p> Student NUMBER: <select id='busNoid' ></select> <br /><br/>Teacher:<input id= 'conid' type='text' /> <br /><br/><button id='submitass'>ADD CO_HOST</button> </div><script> $('#submitass').click( function() {  $.get('Sass',{ 'id': 'assign', 'busNo': document.getElementById('busNoid').value, 			'con': document.getElementById('conid').value  	}).done(function(response) {  alert(response); }); }); $(document).ready(function() {  $.get('Smodify', 		{ 		'id': 'showBusNo' 			} ).done(function(response) {  	$('#busNoid').html(response); }); }); </script>");
             }
            break;
             }}
     		}
