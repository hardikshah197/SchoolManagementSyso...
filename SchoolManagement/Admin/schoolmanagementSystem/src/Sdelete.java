	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    try { 
            ...
            st.close();}
            	...
            	response.getWriter().print("Teacher NOT FOUND");
            	break;
            }
            
            response.getWriter().write("deleted teacher");
            System.out.print("teacher deleted");
            break;
			}
			else if(id.equals("print")){
           	 response.getWriter().write("<hr size='2px'> <div id='del' class = 'section'> <p class='heading'>DELETE Teacher</p> Teacher NUMBER: <input id='busNoid' type='text'/> <br /><br/> <button id='submitdel'>DELETE TEACHER</button> </div> <hr size='2px'><script> $('#submitdel').click( function() {$.get('Sdelete',{ 'id':'deleteBus', 'busNo': document.getElementById('busNoid').value 			}).done(function(response) {alert(response); }); }); </script>");
			}
			break;
            }
    			 }    		
